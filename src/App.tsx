...
const PinModal = ({ isOpen, onClose, onVerify, title, error }: { 
  isOpen: boolean, 
  onClose: () => void, 
      ...
      >
        <div className="bg-primary/95 p-8 text-white flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="bg-accent/20 p-2 rounded-xl">
              <ShieldCheck size={28} className="text-accent shadow-[0_0_15px_rgba(242,125,38,0.5)]" />
            </div>
            <div>
              <h3 className="text-2xl font-bold tracking-tight">{title}</h3>
              <p className="text-white/60 text-xs font-semibold uppercase tracking-widest">Authorization</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-2xl transition-all active:scale-90">
            <X size={24} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          <div className="text-center space-y-2">
            <p className="text-slate-600 font-medium italic">Secure PIN verification required</p>
          </div>
        ...
        <div className="bg-primary/95 p-8 text-white flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/60 text-xs font-semibold uppercase tracking-widest">New Profile</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-2xl transition-all active:scale-90">
            <X size={24} />
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Initial Role</label>
                <select 
                  value={role}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Initial Password (Optional)</label>
                <input 
                  type="password"
                  value={password}
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Secure PIN (4-6 digits)</label>
                <input 
                  type="password"
                  maxLength={6}
                  required
        ...
        <div className="bg-primary/95 p-8 text-white flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/60 text-xs font-semibold uppercase tracking-widest">Portal Settings</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-2xl transition-all active:scale-90">
            <X size={24} />
      ...
      >
        <div className="p-8 border-b border-primary/10 flex justify-between items-center bg-primary text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold tracking-tight">{title}</h3>
              <p className="text-white/60 text-xs font-semibold uppercase tracking-widest">Authentication</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-2xl transition-all active:scale-90">
            <X size={24} />
        ...
        <div className="p-8 space-y-6">
          <p className="text-slate-600 font-medium italic text-center">Please provide your handwritten signature to authorize this log.</p>
...
const BiometricModal = ({ 
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      console.warn("Camera access denied or unavailable, using secure matrix fallback:", err);
      setStatusText('Camera stream blocked. Activating synthetic neural mesh scanner...');
    }
  };
    ...
    if (scanning) {
      interval = setInterval(() => {
        setProgress(prev => {
          ...
          else if (next < 85) setStatusText('Verifying live signature patterns...');
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold tracking-tight">
                {actionType === 'register' ? 'Register Face ID Reference' : 'Face ID Security Access'}
              </h3>
              <p className="text-white/40 text-[10px] font-black uppercase tracking-widest">COTRAC Biometric Portal</p>
            </div>
          </div>
          <button onClick={handleCancel} className="p-2 hover:bg-white/10 rounded-2xl transition-all">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-8 space-y-8">
          <div className="flex flex-col items-center space-y-6">
            <div className="relative w-56 h-56 rounded-full border-4 border-dashed border-accent/40 flex items-center justify-center overflow-hidden bg-slate-900 shadow-2xl">
              {cameraStream ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-40">
                  ...
                  <svg className="w-32 h-32 text-emerald-400 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M10 21h4m1.8-8H8.2m8.6-4a4 4 0 11-8 0 4 4 0 018 0z" />
                  </svg>
                </div>
              )}
            ...
            <div className="w-full space-y-3 text-center">
              ...
              <div className="text-xs text-accent font-mono font-bold animate-pulse uppercase tracking-wider">{progress}% scanned</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 sm:h-24 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl sm:text-2xl font-black text-primary tracking-tighter uppercase italic leading-none">COTRAC</h1>
            <span className="text-[8px] sm:text-[10px] text-accent font-black tracking-[0.3em] uppercase opacity-60">Nexus Portal</span>
          </div>
        </div>
        ...
        {user && (
          <nav className="hidden lg:flex items-center gap-1 bg-slate-100/50 p-1.5 rounded-[1.5rem] border border-white">
            {[
              ...
              ...(user.role === 'admin' ? [{ id: 'admin', label: 'Nexus', icon: ShieldCheck }] : []),
            ].map((item) => (
              <button 
                key={item.id}
                ...
                className={cn(
                  ...
                  activeTab === item.id ? "bg-white text-primary shadow-lg" : "text-slate-400 hover:text-primary hover:bg-white/50"
                )}
              >
                <item.icon size={16} />
                {item.label}
          ...
          {user && (
            <div className="flex items-center gap-2 sm:gap-6">
              <div className="hidden sm:flex flex-col items-end">
                <span className="text-sm font-black text-primary uppercase tracking-tighter leading-none">{user.displayName}</span>
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest bg-slate-100 px-2 py-0.5 rounded mt-1">{user.role}</span>
              </div>
              <button 
          ...
          {[
            ...
            <button
              ...
              className={cn(
                ...
                activeTab === tab.id ? "bg-primary text-white shadow-lg" : "text-slate-400 hover:bg-white"
              )}
            >
              <tab.icon size={18} />
...
const Login = ({ onLogin, onCustomLogin, isLoading, popupBlocked }: { onLogin: () => void, onCustomLogin: (user: UserProfile) => void, isLoading: boolean, popupBlocked: boolean }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-6">
      <motion.div 
        ...
        animate={{ opacity: 1, y: 0 }}
        className="glass rounded-[3rem] max-w-lg w-full text-center space-y-8 p-10 relative overflow-hidden"
      >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <h1 className="text-4xl font-black text-primary tracking-tighter uppercase italic">Staff Portal</h1>
            <p className="text-slate-600 font-medium text-sm leading-tight">Secure clock-in and clock-out system<br/>for COTRAC employees.</p>
          </div>
        </div>
        ...
        <div className="bg-slate-100 p-1 rounded-2xl flex border border-slate-200">
          <button
            ...
            className={cn(
              "flex-1 py-3 text-xs font-black uppercase tracking-wider rounded-xl transition-all duration-200",
              activeTab === 'google' 
          ...
          {activeTab === 'google' ? (
            ...
            >
              ...
              {(popupBlocked || isIframe) && (
                ...
                >
                  <div className="flex items-start gap-2.5 text-amber-700">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase tracking-wider text-amber-600">Popup & Sandbox Check</p>
              ...
              <div className="pt-2 flex flex-col items-center gap-1.5">
                ...
                <p className="text-[10px] text-slate-400 font-medium">Suitable for Google accounts & mojaizs@gmail.com.</p>
              </div>
            </motion.div>
          ) : (
            <motion.div
            ...
            >
              <form onSubmit={handleEmailAuth} className="space-y-4 text-left">
                {emailMode === 'signup' && (
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block px-1 italic">Full Name</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block px-1 italic">Corporate Email</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400">
                      <Mail size={16} />
                    </span>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block px-1 italic">Password</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400">
                      <Lock size={16} />
                    </span>
                ...
                {emailMode === 'signup' && (
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block px-1 italic">Secure PIN (4-6 digits)</label>
                    <div className="relative">
                      <span className="absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400">
                        <Lock size={16} />
                      </span>
    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-2">
          ...
          <h2 className="text-6xl font-black text-primary tracking-tighter uppercase italic leading-none">Welcome, {user.displayName}</h2>
          <p className="text-slate-500 font-medium text-xl">Operational integrity confirmed. Time stream active.</p>
        ...
        <div className="glass px-12 py-8 rounded-[2.5rem] flex items-center gap-8 shadow-2xl relative overflow-hidden group border-white/40">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-accent"></div>
          <div className="bg-primary/5 p-4 rounded-2xl group-hover:rotate-12 transition-transform">
            <Clock className="text-accent" size={40} />
          </div>
          <div className="flex flex-col">
            <span className="text-5xl font-black text-primary tabular-nums tracking-tighter leading-none">{format(time, 'HH:mm:ss')}</span>
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em] mt-2 italic">{format(time, 'EEEE, MMMM do')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Status Card */}
        <div className="glass lg:col-span-2 p-12 rounded-[4rem] flex flex-col justify-between min-h-[400px] relative overflow-hidden bg-white/70 border-white/50">
          ...
          <div className="flex justify-between items-start z-10">
            <div>
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4 italic">Personnel Engagement Status</h3>
              <div className="flex items-center gap-4">
                {isClockedIn ? (
          ...
          <div className="flex flex-wrap gap-6 mt-16 z-10">
            {false ? (
              <div className="flex-1 p-8 bg-slate-100/40 rounded-3xl border border-slate-200/50 text-center flex flex-col items-center justify-center gap-2">
                <ShieldAlert className="text-amber-500" size={32} />
                <div className="space-y-1">
                  <p className="text-sm font-black text-slate-700 uppercase tracking-wider">Awaiting Sign-In Officer Authorization</p>
                  <p className="text-xs text-slate-500 font-semibold leading-normal">Please proceed to the nearest Sign-In Officer Terminal to authorize your check-in or check-out.</p>
                </div>
              </div>
            ) : (
              <>
                {!isClockedIn && !isClockedOut && (
                  <button 
                    onClick={onClockIn}
                    className="flex-[2] btn-primary py-6 text-xl flex items-center justify-center gap-4 shadow-2xl shadow-primary/40 font-black tracking-tight"
                  >
                    <Clock size={28} />
                    AUTHORIZE CHECK-IN
                  </button>
                ...
                {isClockedIn && (
                  <button 
                    onClick={onClockOut}
                    className="flex-[2] btn-accent py-6 text-xl flex items-center justify-center gap-4 shadow-2xl shadow-accent/40 font-black tracking-tight"
                  >
                    <LogOut size={28} />
                    AUTHORIZE CHECK-OUT
                  </button>
            ...
            {(isClockedOut || isClockedIn) && (
              <div className="flex-1 glass-dark rounded-[2.5rem] p-8 flex flex-col justify-center border-white/5 shadow-2xl">
                <span className="text-[10px] text-white/40 uppercase font-black tracking-[0.2em] mb-2 italic">Entry Anchor</span>
                <span className="text-3xl font-black text-white tabular-nums tracking-tighter">
                  {todayRecord ? format(new Date(todayRecord.clockIn), 'HH:mm') : '--:--'}
                </span>
              </div>
            )}
            {isClockedOut && (
              <div className="flex-1 glass-dark rounded-[2.5rem] p-8 flex flex-col justify-center border-white/5 shadow-2xl">
                <span className="text-[10px] text-white/40 uppercase font-black tracking-[0.2em] mb-2 italic">Release Anchor</span>
                <span className="text-3xl font-black text-white tabular-nums tracking-tighter">
        ...
        <div className="space-y-8">
          <div className="glass p-10 rounded-[4rem] bg-primary text-white border-none shadow-2xl relative overflow-hidden group">
            ...
            <h4 className="text-white/40 text-[10px] font-black uppercase tracking-[0.3em] mb-6 italic relative z-10">Productivity Index</h4>
            <div className="flex items-baseline gap-2 relative z-10">
              <span className="text-7xl font-black tracking-tighter italic tabular-nums">
              ...
              <span className="text-xs font-black uppercase tracking-widest opacity-40 italic">Quota Units</span>
            </div>
            ...
            <p className="mt-4 text-[10px] text-white/30 font-bold uppercase tracking-widest italic relative z-10">Rolling 7-day work cycle</p>
          </div>
          
          <div className="glass-dark p-10 rounded-[4rem] text-white border-white/5 shadow-2xl">
            <h4 className="text-white/20 text-[10px] font-black uppercase tracking-[0.3em] mb-8 italic">Stream Pulse</h4>
            <div className="space-y-6">
              {statsRecords.slice(0, 4).map((record, i) => (
                <div key={i} className="flex justify-between items-center group">
                  <div className="flex flex-col">
                    <span className="text-sm font-black text-white tracking-tight">{format(new Date(record.date), 'MMMM dd')}</span>
                    <span className={cn(
                      "text-[9px] font-black uppercase tracking-widest",
                      record.status === 'Present' ? "text-emerald-400" : record.status === 'Late' ? "text-amber-400" : "text-rose-400"
                    )}>{record.status}</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <div className="text-xl font-black text-white tabular-nums tracking-tighter">{(record.totalHours || 0).toFixed(1)}H</div>
                    <div className={cn(
                      "h-1 w-16 rounded-full mt-1.5 opacity-30 group-hover:opacity-100 transition-opacity shadow-[0_0_10px_currentcolor]",
                      record.status === 'Present' ? "bg-emerald-400" : record.status === 'Late' ? "bg-amber-400" : "bg-rose-400"
                    )}></div>
                  </div>
                </div>
              ))}
              {statsRecords.length === 0 && (
                <p className="text-xs text-white/20 font-black uppercase tracking-widest italic text-center py-8">Static signal detected.</p>
              )}
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary/80 backdrop-blur-md text-white uppercase text-[10px] font-black tracking-[0.2em] italic">
                ...
                {isAdmin && <th className="px-8 py-6 text-center">Personnel</th>}
                <th className="px-8 py-6 text-center">Duty Start</th>
                <th className="px-8 py-6 text-center">Duty End</th>
                <th className="px-8 py-6 text-center border-x border-white/5">Quota Units</th>
                <th className="px-8 py-6 text-center">Status</th>
                <th className="px-8 py-6 text-center">Auth Sig {isAdmin && "(vs Ref)"}</th>
            ...
            <tbody className="text-white/80">
              {records.length === 0 ? (
                <tr>
                  <td colSpan={isAdmin ? 8 : 6} className="px-8 py-20 text-center text-white/30 font-bold italic uppercase tracking-widest">
                ...
                records.map((record, idx) => {
                  const staffProfile = users?.find(u => u.uid === record.userId);
                  return (
                    <tr key={record.id || idx} className="border-b border-white/5 hover:bg-white/5 transition-all group">
                      ...
                      <td className="px-8 py-6 text-center">
                        <span className={cn(
                          ...
                          record.status === 'Incomplete' && "bg-rose-500/20 text-rose-400 border border-rose-500/30"
                        )}>
                          {record.status}
                        </span>
                      </td>
                      <td className="px-8 py-6 text-center">
                        <div className="flex justify-center gap-2 items-center">
                            ...
                            <div className="group/ref relative mr-2 border-r border-white/10 pr-2">
                              ...
                              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/ref:block z-50">
                                <div className="bg-slate-900 p-2 rounded-xl border border-white/20 whitespace-nowrap shadow-2xl">
                                  <p className="text-[8px] font-black uppercase tracking-widest text-accent mb-1">Official Reference</p>
                                  <img src={staffProfile.registeredSignature} className="h-20 w-32 object-contain invert" />
                                </div>
                              </div>
                            </div>
                          )}
                            ...
                            <div className="flex flex-col items-center gap-1.5">
                              <div className="group/sig relative">
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 hidden group-hover/sig:block z-50">
                                  ...
                                  >
                                    <img src={record.clockInSignature} alt="SIG-IN-LG" className="h-40 w-60 object-contain invert" />
                                    <p className="text-[10px] text-white/40 text-center font-black uppercase tracking-widest mt-2 italic">Entry Verification</p>
                                  </motion.div>
                                </div>
                              </div>
                              {record.biometricVerified && (
                                <div className="group/bio relative cursor-pointer">
                                  <span className="flex items-center gap-1 px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[8px] font-black uppercase tracking-tight rounded-lg">
                                    ...
                                    {record.biometricType === 'fingerprint' ? 'Touch ID' : 'Face ID'}
                                  </span>
                                  {record.biometricStamp && (
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/bio:block z-50">
                                      <div className="bg-slate-900 p-2.5 rounded-2xl border border-white/20 shadow-2xl w-48">
                                        <p className="text-[8px] font-black uppercase tracking-widest text-accent mb-1.5 text-center">Biometric Receipt</p>
                                        <img src={record.biometricStamp} className="h-20 w-full object-contain" />
                                      </div>
                                    </div>
                                  )}
                                </div>
                            ...
                            <div className="flex flex-col items-center gap-1.5">
                              <div className="group/sig relative">
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 hidden group-hover/sig:block z-50">
                                  <motion.div 
                                    ...
                                    className="bg-slate-900 p-3 rounded-[1.5rem] shadow-2xl border border-white/10"
                                  >
                                    <img src={record.clockOutSignature} alt="SIG-OUT-LG" className="h-40 w-60 object-contain invert" />
                                    <p className="text-[10px] text-white/40 text-center font-black uppercase tracking-widest mt-2 italic">Exit Verification</p>
                                  </motion.div>
                                </div>
                              </div>
                              {record.clockOutBiometricVerified && (
                                <div className="group/bio-out relative cursor-pointer">
                                  <span className="flex items-center gap-1 px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[8px] font-black uppercase tracking-tight rounded-lg">
                                    <ShieldCheck size={10} className="shrink-0" />
                                    {record.clockOutBiometricType === 'fingerprint' ? 'Touch ID' : 'Face ID'}
                                  </span>
                                  {record.clockOutBiometricStamp && (
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/bio-out:block z-50">
                                      <div className="bg-slate-900 p-2.5 rounded-2xl border border-white/20 shadow-2xl w-48">
                                        <p className="text-[8px] font-black uppercase tracking-widest text-accent mb-1.5 text-center">Biometric Receipt</p>
                                        <img src={record.clockOutBiometricStamp} className="h-20 w-full object-contain" />
                                      </div>
                                    </div>
                                  )}
                                </div>
                          ...
                          {isAdmin && staffProfile?.registeredSignature && record.clockInSignature && (
                              ...
                              {record.signatureMatchPercentage !== undefined ? (
                                <div className="relative group/verify">
                                  <span className={cn(
                                    ...
                                    AI: {record.signatureMatchPercentage}% {record.signatureMatchVerified ? "✓" : "⚠"}
                                  </span>
                                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover/verify:block z-50">
                                    <div className="bg-slate-900 p-4 rounded-3xl border border-white/10 w-72 shadow-2xl text-left">
                                      <p className="text-[9px] font-black uppercase tracking-wider text-accent mb-2">Forensic AI Analysis</p>
                                      <div className="mb-3 flex justify-between items-center bg-white/5 p-2 rounded-xl border border-white/5">
                                        <span className="text-[10px] text-white/60 font-medium">Correlation Score:</span>
                                        <span className={cn(
                                          "text-xs font-black",
                                          record.signatureMatchVerified ? "text-emerald-400" : "text-rose-400"
                                        )}>{record.signatureMatchPercentage}%</span>
                                      </div>
                                      <p className="text-[11px] text-slate-300 leading-relaxed font-semibold italic">"{record.signatureMatchReason}"</p>
                                      <button
      ...
      <div className="lg:hidden space-y-4">
        {records.length === 0 ? (
          <div className="glass p-12 text-center rounded-[2rem]">
            <p className="text-xs text-slate-400 font-black uppercase tracking-widest italic">No registered logs in database.</p>
          </div>
        ) : (
          records.map((record, idx) => {
            const staffProfile = users?.find(u => u.uid === record.userId);
            return (
              <div key={record.id || idx} className="glass rounded-[2rem] p-6 space-y-4 relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-4">
                  <span className={cn(
                    "px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter",
                    record.status === 'Present' && "bg-emerald-500/20 text-emerald-600",
                    record.status === 'Late' && "bg-amber-500/20 text-amber-600",
                    record.status === 'Incomplete' && "bg-rose-500/20 text-rose-600"
                  )}>
                    {record.status}
                  </span>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest italic">{format(new Date(record.date), 'MMMM dd, yyyy')}</span>
                  {isAdmin && <span className="text-lg font-black text-primary leading-tight">{record.employeeName}</span>}
                </div>

                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Entry</span>
                    <span className="text-sm font-black tabular-nums">{format(new Date(record.clockIn), 'HH:mm:ss')}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Exit</span>
                    <span className="text-sm font-black tabular-nums">{record.clockOut ? format(new Date(record.clockOut), 'HH:mm:ss') : '--:--:--'}</span>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                  <div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Total Hours</span>
                    <span className="text-xl font-black tabular-nums text-accent">{record.totalHours ? record.totalHours.toFixed(1) : '0.0'}H</span>
                  </div>
                  <div className="flex gap-2 items-center">
                    {isAdmin && staffProfile?.registeredSignature && record.clockInSignature && (
                      <div className="mr-2">
                        {record.signatureMatchPercentage !== undefined ? (
                          ...
                          <div>
                            {verifyingMap[record.id!] ? (
                              <span className="text-[8px] uppercase tracking-widest font-black text-accent animate-pulse">Running</span>
                            ) : (
                              <button
                                onClick={() => handleTriggerVerify(record)}
                    ...
                    {record.clockInSignature && (
                      <div className="flex items-center gap-1">
                        <div className="w-10 h-8 bg-white/50 rounded-lg border border-slate-200 flex items-center justify-center p-1" title="Clock In Signature">
                          <img src={record.clockInSignature} className="max-h-full max-w-full object-contain" />
                        </div>
                        {record.biometricVerified && (
                          <div className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-600 border border-emerald-500/30 rounded-lg text-[8px] font-black" title="Biometric Verified Clock-In">
                            {record.biometricType === 'fingerprint' ? 'Touch' : 'Face'}
                          </div>
                        )}
                      </div>
                    )}
                    {record.clockOutSignature && (
                      <div className="flex items-center gap-1">
                        <div className="w-10 h-8 bg-white/50 rounded-lg border border-slate-200 flex items-center justify-center p-1" title="Clock Out Signature">
                          <img src={record.clockOutSignature} className="max-h-full max-w-full object-contain" />
                        </div>
                        {record.clockOutBiometricVerified && (
                          <div className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-600 border border-emerald-500/30 rounded-lg text-[8px] font-black" title="Biometric Verified Clock-Out">
                            {record.clockOutBiometricType === 'fingerprint' ? 'Touch' : 'Face'}
                          </div>
                        )}
                      </div>
                    )}
    ...
    <div className="space-y-8 sm:space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-2 sm:px-0">
        <div className="space-y-2">
          ...
          <h2 className="text-4xl sm:text-6xl font-black text-primary tracking-tighter uppercase italic leading-none">Admin Nexus</h2>
          <p className="text-slate-500 font-medium text-lg sm:text-xl">Monitor logistical patterns and flow.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          ...
          <button 
            ...
            className={cn(
              ...
              showUsers ? "bg-primary text-white border-primary shadow-primary/30" : "glass text-slate-600 border-white/40 hover:bg-white/90"
            )}
          >
            {showUsers ? (
      ...
      <div className="glass rounded-[2rem] sm:rounded-[3rem] bg-white/70 border-white/40 shadow-xl overflow-hidden">
        <div className="p-6 sm:p-10 border-b border-slate-100">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 sm:gap-8">
            <div className="relative">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block px-2 italic font-sans sans-serif">Signal Search</label>
              <div className="relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
                <input 
                  type="text" 
                  placeholder={showUsers ? "ID, Name or Email..." : "Employee ID or Name..."}
            ...
            {!showUsers && (
              <>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block px-2 italic">Range Start</label>
                  <div className="relative">
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block px-2 italic">Range End</label>
                  <div className="relative">
                    <CalendarIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
                    <input 
                      type="date" 
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block px-1 italic">Log Filter</label>
                  <div className="relative">
        ...
        {showUsers ? (
          <div className="overflow-x-auto">
            ...
            <table className="hidden lg:table w-full text-left border-collapse">
              ...
              <tbody className="text-white/80">
                {filteredUsers.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-10 py-20 text-center text-slate-400 font-bold uppercase tracking-widest italic">No personnel matches found.</td>
                  </tr>
                ) : (
                  filteredUsers.map((u) => (
                    <tr key={u.uid} className="border-b border-white/5 bg-slate-900/50 hover:bg-slate-900/40 transition-all group">
                      <td className="px-10 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest italic">{u.employeeId || 'ID UNASSIGNED'}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-10 py-6 text-sm font-medium text-white/60">{u.email}</td>
                      <td className="px-10 py-6 text-center">
                        <span className={cn(
                          "px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest",
                          u.role === 'admin' ? "bg-purple-500/20 text-purple-400 border border-purple-500/30" : "bg-white/5 text-white/40 border border-white/10"
                        )}>
                          {u.role}
                        </span>
                      </td>
            ...
            <div className="lg:hidden p-4 space-y-4">
              {filteredUsers.length === 0 ? (
                <p className="py-20 text-center text-slate-400 font-black uppercase tracking-widest italic text-xs">No personnel matches.</p>
              ) : (
                filteredUsers.map((u) => (
                  <div key={u.uid} className="bg-slate-900/50 p-6 rounded-[2rem] border border-white/5 space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          <span className="text-base font-black text-white">{u.displayName}</span>
                          <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{u.employeeId || 'ID UNASSIGNED'}</span>
                        </div>
                      </div>
                      <select 
                        value={u.role}
                    ...
                    <div className="pb-4 border-b border-white/5">
                      <div className="space-y-1">
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">Duty Start</span>
                        <input 
                          type="time"
                          value={u.shiftStart || '09:00'}
                          onChange={(e) => onUpdateShift(u.uid, e.target.value)}
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <p className="text-xs font-medium text-white/40 truncate flex-1">{u.email}</p>
        ...
        {editingUser && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-primary/20 backdrop-blur-md">
            ...
            >
              ...
              <div className="space-y-8">
                <div className="space-y-2">
                  <h3 className="text-3xl font-black text-primary tracking-tighter uppercase italic leading-none">Edit Identity</h3>
                  <p className="text-slate-500 font-medium italic">Update core personnel authorization data.</p>
                </div>

                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block px-2 italic">Active Personnel Identity</label>
                    <input 
                      type="text"
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block px-2 italic">Assigned Employee Code</label>
                    <input 
                      type="text"
                      className="w-full input-glass px-6 py-4 font-black"
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block px-2 italic">Personnel Authorization Role</label>
                    <select 
...
export default function App() {
  ...
  const [bioAction, setBioAction] = useState<'register' | 'clockIn' | 'clockOut' | 'registerSignature' | null>(null);
        ...
        if (userDoc.exists()) {
          ...
          if (emailDoc.exists()) {
            ...
            try {
              ...
              console.error("Error deleting old pre-registration document:", err);
            }
            setUser(newProfile);
          } else {
  ...
  useEffect(() => {
    ...
    if (user.role === 'admin' || user.role === 'sign-in') {
      const qAll = query(collection(db, 'attendance'), orderBy('clockIn', 'desc'));
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-blocked' || (error.message && error.message.includes('popup-blocked'))) {
  ...
  const onClockInSave = async (signature: string, bioStamp?: string, bioType?: 'face' | 'fingerprint') => {
    ...
    const newRecord: AttendanceRecord = {
      ...
      ...(selectedStaff ? { authorizedBy: user.uid, authorizedByName: user.displayName } : {})
    };
  ...
  const onClockOutSave = async (signature: string, bioStamp?: string, bioType?: 'face' | 'fingerprint') => {
    ...
    const todayRecord = targetRecords.find(r => r.userId === targetUser.uid && r.date === today && !r.clockOut);
  ...
  const handlePurgeDatabase = async () => {
    ...
    if (!confirm('WARNING: You are about to delete ALL non-admin personnel records and ALL attendance history from Firestore. This action is permanent and irreversible. Do you want to proceed?')) {
      return;
    }

    if (!confirm('FINAL CONFIRMATION: Type "PURGE" in the next prompt if you are absolutely sure.')) {
      return;
    }
    ...
    try {
      ...
      alert('Database successfully purged. All non-administrative personnel and attendance records have been deleted.');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <AnimatePresence mode="wait">
          {!user ? (
            <Login onLogin={handleLogin} onCustomLogin={handleCustomLogin} isLoading={isLoggingIn} popupBlocked={popupBlocked} />
          ) : (
            <motion.div
              key={activeTab}
              initial={{ opacity: 0, x: 10 }}
                    ...
                    <div className="pt-6 border-t border-slate-100">
                      ...
                      <div className="bg-slate-50 rounded-2xl p-6 space-y-4">
                        ...
                        <div className="pt-4 border-t border-slate-200 flex justify-between items-center">
                          <div>
                            <p className="font-semibold text-slate-700">Biometric Identity</p>
                            <p className="text-xs text-slate-500">Link Face ID or Touch ID for frictionless logging.</p>
                          </div>
                          <button 
                            onClick={() => {
                            ...
                            className="bg-accent/10 hover:bg-accent/20 text-accent border border-accent/20 py-2 px-4 rounded-xl text-sm font-black transition-all active:scale-95"
                          >
                            {user.biometricsEnabled ? 'Manage Biometrics' : 'Activate Biometrics'}
                          </button>
                        </div>
                        {user.biometricsEnabled && (
                          <div className="flex items-center gap-2 text-emerald-600 text-xs font-semibold">
                            <CheckCircle2 size={14} />
                            Holographic Biometrics Active ({user.biometricType === 'face' ? 'Face ID' : 'Touch ID'})
                          </div>
                        )}
                      </div>
                    </div>
