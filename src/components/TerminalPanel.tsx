...
import { dbSaveRecord } from '../services/databaseService';
...
export default function TerminalPanel({ users, records, onAuthorizeClockIn, onAuthorizeClockOut, onVisitorRecordChange }: TerminalPanelProps) {
  ...
  const handleVisitorCheckIn = async (e: React.FormEvent) => {
    ...
    const cleanName = sanitizeInput(visitorName, 60);
    const cleanEmail = sanitizeInput(visitorEmail, 80);
    const cleanHost = sanitizeInput(visitorHost, 60);
    const cleanPurpose = sanitizeInput(visitorPurpose, 60);
    ...
    const newVisitorRecord: AttendanceRecord = {
      ...
      visitorHost: cleanHost,
      visitorPurpose: cleanPurpose
    };
    ...
    try {
      await dbSaveRecord(newVisitorRecord);
  ...
  const handleVisitorCheckOut = async () => {
    ...
    try {
      await dbSaveRecord(updatedVisitorRecord);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-4">
            {[
              ...
              <button
                ...
                className={cn(
                  "border p-3.5 sm:p-5 rounded-2xl text-left transition-all active:scale-95 flex flex-col justify-between h-28 sm:h-32 bg-white shadow-xs",
                  statusFilter === item.key 
                    ? `${item.color} ring-2 ring-primary/20` 
                    : 'border-slate-200 hover:border-slate-300 text-slate-600'
                )}
              >
                <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider">{item.label}</span>
                <span className="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2 tabular-nums">{item.count}</span>
              </button>
            ))}
          </div>
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-4">
            ...
            <button
              ...
              className="border-2 border-dashed border-primary hover:bg-blue-50/50 text-primary p-3.5 sm:p-5 rounded-2xl text-left transition-all active:scale-95 flex flex-col justify-between h-28 sm:h-32 bg-white shadow-xs group"
            >
              <div className="flex justify-between items-center w-full">
                <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-primary">New Visitor</span>
                <Plus size={18} className="text-primary group-hover:rotate-90 transition-transform" />
              </div>
              <div>
                <span className="text-sm sm:text-base font-bold block text-slate-800 group-hover:text-primary transition-colors">Register Guest</span>
                <p className="text-[11px] sm:text-xs text-slate-400 font-medium mt-0.5 sm:mt-1">Issue entry pass</p>
              </div>
            </button>
          </div>
        ...
        {selectedVisitorForCheckOut && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/40 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto">
            <motion.div 
              ...
              className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200 max-w-md w-full overflow-hidden shadow-2xl flex flex-col max-h-[90vh] my-auto"
            >
              {/* Header */}
              <div className="p-4 sm:p-6 border-b border-slate-100 flex justify-between items-center bg-amber-600 text-white shrink-0">
                <div className="flex items-center gap-2.5 sm:gap-3">
                  <LogOut size={20} className="sm:w-[22px] sm:h-[22px]" />
                  <div>
                    <h3 className="text-base sm:text-lg font-bold">Guest Release Gate</h3>
                    <p className="text-white/80 text-[11px] sm:text-xs">Authorize Check-Out Exit</p>
                  </div>
                </div>
                <button 
