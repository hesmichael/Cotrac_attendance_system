...
import { getSupabase, mapSupabaseToRecord } from '../lib/supabase';
...
export default function ReportsPanel({ records, users, userRole }: ReportsPanelProps) {
  ...
  // Execute on-demand static query for historical dates via Supabase
  const handleFetchHistoricalRange = async () => {
    ...
    try {
      ...
      if (supabase) {
        ...
        if (!error && data) {
          const fetched = data.map(mapSupabaseToRecord);
      ...
      // Offline fallback: filter local active records
      const localFiltered = records.filter(r => r.date >= startStr && r.date <= endStr);
      setHistoricalRecords(localFiltered);
      setLastQueriedRange(`${startStr} to ${endStr}`);
    } catch (err: any) {
      console.warn("Targeted date query fallback:", err);
      setHistoricalRecords(records);
      setLastQueriedRange('Cached Dataset');
    } finally {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 sm:gap-6 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 w-full md:w-auto">
          <button
            ...
            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs sm:text-sm shadow-xs transition-all active:scale-95 min-h-[44px]"
          ...
          <button
            ...
            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm shadow-xs transition-all active:scale-95 min-h-[44px]"
          ...
          <button
            onClick={handlePrint}
            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:py-3 rounded-2xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs sm:text-sm shadow-xs transition-all active:scale-95 min-h-[44px]"
